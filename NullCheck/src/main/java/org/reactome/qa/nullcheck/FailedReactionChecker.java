...
public class FailedReactionChecker implements QACheck
{
	...
	private Report report(String schemaClass, String attribute, String operator, List<Long> skipList)
	{
		...
		instances.addAll(getInstances(this.adaptor, schemaClass, attribute, operator, skipList));

		for (GKInstance instance : instances)
		{
			r.addLine(Arrays.asList(instance.getDBID().toString(), instance.getDisplayName(), instance.getSchemClass().getName(), attribute + " " + operator  , getLastModificationAuthor(instance)));
		}
